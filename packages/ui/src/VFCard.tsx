...
export function VFCard({
  ...
  return (
    <div
      className={cn(
        "rounded-xl border border-border bg-card p-5 sm:p-6 text-card-foreground shadow-xs transition-all duration-200",
        className
      )}
      {...props}
    >
      {(title || description || actions) && (
        <div className={cn("flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-3 mb-3.5 border-b border-border/70", headerClassName)}>
          <div>
