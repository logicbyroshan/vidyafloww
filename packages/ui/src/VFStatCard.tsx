...
export function VFStatCard({
  ...
  return (
    <div
      className={cn(
        "rounded-lg border border-border/70 bg-card p-4 text-card-foreground flex flex-col justify-between relative overflow-hidden transition-all duration-200 hover:border-primary/35 hover:shadow-sm group min-w-0",
        className
      )}
      {...props}
    >
      <div className="flex items-start justify-between gap-2 mb-2.5 min-w-0">
        <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest leading-snug truncate" title={title}>
          {title}
        </span>
        {icon && (
      ...
      <div className="flex flex-col gap-1 min-w-0">
        {isLoading ? (
          ...
          <div className="text-2xl font-black tracking-tight text-foreground leading-none truncate">{value}</div>
        )}
        ...
        {!isLoading && (trend || displayLabel) && (
          <div className="flex items-center gap-1.5 mt-2 flex-wrap min-w-0">
            <span
              className={cn(
                "inline-flex items-center text-xs font-bold rounded-md px-2 py-0.5 border uppercase tracking-wide truncate max-w-full",
            ...
            >
              {trend === 'up' && (
                <svg className="h-2.5 w-2.5 mr-1 stroke-[3] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {trend && description && trendLabel && (
              <span className="text-xs text-muted-foreground font-medium truncate">{description}</span>
            )}
          </div>
        )}
      </div>
