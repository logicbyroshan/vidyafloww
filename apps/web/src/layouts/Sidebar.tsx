  ...
  return (
    ...
    >
                  ...
                  >
                    ...
                    <span
                      className={cn(
                        ...
                        sidebarExpanded ? 'opacity-100 max-w-[160px] ml-3 font-semibold' : 'opacity-0 max-w-0 ml-0 pointer-events-none'
                      )}
                    >
                      {item.label}
                    </span>
      ...
      <div className="p-3 border-t border-border bg-card/60 shrink-0">
        <div className={cn('flex items-center w-full min-w-0', sidebarExpanded ? 'gap-3' : 'justify-center')}>
          <VFAvatar fallback="Roshan Singh" size="sm" className="h-10 w-10 text-sm shrink-0 rounded-lg" />
          <div
            className={cn(
              'flex-1 min-w-0 transition-all duration-300 ease-in-out overflow-hidden whitespace-nowrap',
              sidebarExpanded ? 'opacity-100 max-w-[120px] ml-3' : 'opacity-0 max-w-0 pointer-events-none hidden'
            )}
          >
            <p className="text-base font-bold text-foreground truncate leading-none">Roshan Singh</p>
            <p className="text-xs text-muted-foreground font-bold truncate mt-1">Super Admin</p>
          </div>
          <Link
            to="/login"
            className={cn(
