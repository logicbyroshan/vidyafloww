...
export function CommandPalette({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <VFDialog
      ...
      onClose={onClose}
      hideHeader={true}
      className="p-0 max-w-3xl bg-card/95 backdrop-blur-2xl border border-border/80 rounded-2xl overflow-hidden shadow-2xl mt-16 animate-scale-in"
    >
      <div className="flex flex-col h-full -m-6 divide-y divide-border/60" onKeyDown={handleKeyDown}>
        {/* Top Search Input Bar - Modal starts directly here */}
        <div className="flex items-center px-5 py-4 bg-card">
          <Search className="h-5 w-5 text-primary mr-3.5 shrink-0" />
          <input
            type="text"
            autoFocus
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="flex-1 h-11 bg-transparent outline-none text-sm md:text-base text-foreground placeholder:text-muted-foreground font-medium"
            placeholder="Type a command or search across all 20 ERP modules..."
          />
          <kbd className="hidden sm:inline-flex h-6 select-none items-center gap-1 rounded border border-border/80 bg-muted px-2 font-mono text-xs font-semibold text-muted-foreground">
            ESC
          </kbd>
        </div>

        {/* Command List Scroll View */}
        <div className="max-h-[420px] overflow-y-auto p-3 space-y-1.5 custom-scrollbar">
          {filtered.length === 0 ? (
            <div className="py-16 text-center text-sm text-muted-foreground space-y-1.5">
              <Search className="h-10 w-10 text-muted-foreground/40 mx-auto mb-2" />
              <p className="font-semibold text-foreground text-base">No matching commands found</p>
              <p className="text-xs">Try searching for "Admissions", "Fees", "Timetable", or "AI"</p>
            </div>
          ) : (
            filtered.map((cmd, idx) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all outline-none text-xs md:text-sm text-left cursor-pointer ${
                    isSelected
                ...
                >
                  ...
                  <div className="flex items-center gap-2.5 shrink-0">
                    <span className="text-xs font-mono text-muted-foreground bg-muted/60 px-2 py-0.5 rounded border border-border/60">
                      {cmd.shortcut}
                    </span>
                    <VFBadge variant={isSelected ? 'primary' : 'outline'} className="text-xs px-2.5 py-0.5">
                      {cmd.category}
                    </VFBadge>
                    <ArrowRight className={`h-4 w-4 transition-transform ${isSelected ? 'translate-x-0.5 text-primary' : 'opacity-0'}`} />
                  </div>
                </button>
              );
            })
        ...
        <div className="px-5 py-3 bg-muted/30 flex items-center justify-between text-xs text-muted-foreground">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1.5">
              <kbd className="px-2 py-0.5 rounded bg-muted border border-border font-mono text-xs font-bold text-foreground">↑↓</kbd> Navigate
            </span>
            <span className="flex items-center gap-1.5">
              <kbd className="px-2 py-0.5 rounded bg-muted border border-border font-mono text-xs font-bold text-foreground">↵</kbd> Select
            </span>
          </div>

          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5 text-primary font-bold text-xs mr-2">
            ...
            <button
              onClick={onClose}
              className="px-3 py-1 rounded-lg bg-muted hover:bg-destructive/15 text-muted-foreground hover:text-destructive border border-border hover:border-destructive/30 transition-all flex items-center gap-1 text-xs font-semibold cursor-pointer"
              title="Close palette"
            >
